...
public class Triangulator : IDisposable
{
	...
	private struct ValidateInputPositionsJob(Triangulator triangulator) : IJob
	{
		...
		public void Execute()
		{
			...
			{
				UnityEngine.Debug.LogError("[Triangulator]: Positions.Length is less then 3!");
			...
			for (int i = 0; i < positions.Length; i++)
			{
				...
				{
					UnityEngine.Debug.LogError($"[Triangulator]: Positions[{i}] does not contain finite value: {positions[i]}!");
					status.Value |= Status.ERR;
				}
		...
		private bool PointPointValidation(int i)
		{
			...
			for (int j = i + 1; j < positions.Length; j++)
			{
				...
				{
					UnityEngine.Debug.LogError($"[Triangulator]: Positions[{i}] and [{j}] are duplicated with value: {float5}!");
					return false;
				}
			}
			return true;
							...
							using (ids = new NativeArray<int>(length, Allocator.Temp))
							{
								using (dists = new NativeArray<float>(length, Allocator.Temp))
								{
									using (EDGE_STACK = new NativeArray<int>(512, Allocator.Temp))
									{
										...
										{
											UnityEngine.Debug.LogError("[Triangulator]: Provided input is not supported!");
											status.Value |= Status.ERR;
											return;
										}
										int num11;
	...
	private struct ValidateInputConstraintEdges(Triangulator triangulator) : IJob
	{
		...
		public void Execute()
		{
			...
			{
				UnityEngine.Debug.LogError("[Triangulator]: Constraint input buffer does not contain even number of elements!");
				status.Value |= Status.ERR;
				return;
			}
		...
		private bool EdgePositionsRangeValidation(int i)
		{
			...
			{
				UnityEngine.Debug.LogError($"[Triangulator]: ConstraintEdges[{i}] = ({num3}, {num4}) is out of range Positions.Length = {length}!");
				return false;
			}
			return true;
		}
		...
		private bool EdgeValidation(int i)
		{
			...
			if (num3 == num4)
			{
				UnityEngine.Debug.LogError($"[Triangulator]: ConstraintEdges[{i}] is length zero!");
				return false;
			}
			return true;
		}
		...
		private bool EdgePointValidation(int i)
		{
			...
			for (int j = 0; j < positions.Length; j++)
			{
				...
				{
					UnityEngine.Debug.LogError($"[Triangulator]: ConstraintEdges[{i}] and Positions[{j}] are collinear!");
					return false;
				}
			}
			return true;
		...
		private bool ValidatePair(int i, int j)
		{
			...
			{
				UnityEngine.Debug.LogError($"[Triangulator]: ConstraintEdges[{i}] and [{j}] are equivalent!");
				return false;
			}
			...
			{
				UnityEngine.Debug.LogError($"[Triangulator]: ConstraintEdges[{i}] and [{j}] intersect!");
				return false;
			}
			return true;
		}
	...
	private struct ConstrainEdgesJob(Triangulator triangulator) : IJob
	{
		...
		private bool IsMaxItersExceeded(int iter, int maxIters)
		{
			if (iter >= maxIters)
			{
				UnityEngine.Debug.LogError("[Triangulator]: Sloan max iterations exceeded! This may suggest that input data is hard to resolve by Sloan's algorithm. It usually happens when the scale of the input positions is not uniform. Please try to post-process input data or increase SloanMaxIters value.");
				status.Value |= Status.ERR;
				return true;
			}
			return false;
